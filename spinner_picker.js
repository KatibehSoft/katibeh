class SpinnerPicker {
    ...
    constructor(canvas, valueHandler, config, onchanged=null) {
        ...
        this.backColor = (config != null && "back_color" in config) ? config["back_color"] : getComputedStyle(document.body).getPropertyValue('--backgroundcolor');
        this.font = (config != null && "font" in config) ? config["font"] :"Titr";
...
//        this.drawLine([[0, hBoxSize * 3], [this.canvas.width, hBoxSize * 3]], 4, this.selectionColor, 50, this.selectionColor);
  //      this.drawLine([[0, hBoxSize * 4], [this.canvas.width, hBoxSize * 4]], 4, this.selectionColor, 50, this.selectionColor);
        this.drawPolygon([[0, 0], [this.canvas.width, 0], [this.canvas.width, hBoxSize * 3], [0, hBoxSize * 3]],this.backColor);
        this.drawPolygon([[0, hBoxSize * 4], [this.canvas.width, hBoxSize * 4], [this.canvas.width, this.canvas.height], [0, this.canvas.height]], this.backColor);
    //    this.drawLine([[0, hBoxSize * 3], [this.canvas.width, hBoxSize * 3]], 4, this.selectionColor);
      //  this.drawLine([[0, hBoxSize * 4], [this.canvas.width, hBoxSize * 4]], 4, this.selectionColor);
        for(var i=-4; i<=4; i++) {
            ...
            if(value != null) {
                this.drawText(String(value), [horizontalCenter, y], this.fontColor, hBoxSize-20 + "px " + this.font, 0, Math.abs(y - (3.55 * hBoxSize)) / (4 * hBoxSize));
            }
        }
        if(this.currentAnimationStep != this.animationSteps) {
